...
func (p *certServiceContextProvider) GetObjectContext(ctx context.Context, reqData oathkeeper.ReqData, authDetails oathkeeper.AuthDetails) (ObjectContext, error) {
	...
	if err != nil {
		if directorErrors.IsGQLNotFoundError(err) {
			...
			return NewObjectContext(NewTenantContext(externalTenantID, ""), p.tenantKeys, scopes, mergeWithOtherScopes, authDetails.Region, "", authDetails.AuthID, authDetails.AuthFlow, consumer.ResourceProvider, tenantmapping.CertServiceObjectContextProvider), nil
		}

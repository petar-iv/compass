...
// ListByScenarios provides a mock function with given fields: ctx, scenarios, pageSize, cursor
func (_m *ApplicationService) ListByScenarios(ctx context.Context, scenarios []string, pageSize int, cursor string) (*model.ApplicationPage, error) {
	ret := _m.Called(ctx, scenarios, pageSize, cursor)

	var r0 *model.ApplicationPage
	if rf, ok := ret.Get(0).(func(context.Context, []string, int, string) *model.ApplicationPage); ok {
		r0 = rf(ctx, scenarios, pageSize, cursor)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*model.ApplicationPage)
		}
	...
	if rf, ok := ret.Get(1).(func(context.Context, []string, int, string) error); ok {
		r1 = rf(ctx, scenarios, pageSize, cursor)
	} else {
		r1 = ret.Error(1)
	}

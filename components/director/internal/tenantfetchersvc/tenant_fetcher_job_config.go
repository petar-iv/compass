...
// GetICPCgf get configuration for handling entitlements
func (cfg *JobConfig) GetICPCgf() tenantfetcher.ICPConfig {
	return cfg.icpConfig
}
...
func (j *job) readHandlerConfig() HandlerConfig {
	return HandlerConfig{
		...
		ShouldSyncCustomers:          parseBool(j.getEnvValueForKey("false", "APP_"+j.name+"_SYNC_CUSTOMERS")),
...
func (j *job) readICPConfig() tenantfetcher.ICPConfig {
	return tenantfetcher.ICPConfig{
		TokenURL:              j.getEnvValueForKey(emptyValue, "APP_"+j.name+"_ICP_TOKEN_URL"),
		EntitlementsURL:       j.getEnvValueForKey(emptyValue, "APP_"+j.name+"_ENTITLEMENTS_URL"),
		GetTokenAuthorization: j.getEnvValueForKey(emptyValue, "APP_"+j.name+"_ICP_GET_TOKEN_AUTH"),
	}
}
